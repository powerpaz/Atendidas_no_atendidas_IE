...
// Actualiza tamaño/visibilidad de etiquetas de provincias según el zoom
map.on('zoomend', () => updateProvinceLabelsByZoom(map));
map.on('moveend', () => updateProvinceLabelsByZoom(map));
...
map.getPane('paneLabels').style.zIndex = 520;
...
// ---------- Labels de provincias (DPA_DESPRO) ----------
let _provLabelMarkers = [];

function _labelFontSize(zoom) {
  // Escala tipo "ArcGIS-like": más grande cuando acercas, sin volverse gigante
  const z = Number(zoom) || 6;
  const size = 10 + (z - 6) * 1.6; // 6->10px, 10->16.4px, 12->19.6px aprox
...
function buildProvinceLabels(geojson) {
  ...
  feats.forEach((f) => {
    ...
    const name = p.DPA_DESPRO || p.NOMBRE || p.nombre || '';
    if (!name) return;

    // Centro por bounds (robusto para MultiPolygon)
    ...
    const mk = L.marker(c, {
      ...
      icon: L.divIcon({
        ...
        html: `<span>${String(name).toUpperCase()}</span>`
      })
    });

    mk.addTo(group);
    _provLabelMarkers.push(mk);
  });

  return group;
}

function updateProvinceLabelsByZoom(map) {
  if (!_provLabelMarkers || _provLabelMarkers.length === 0) return;
  const z = map.getZoom();
  const fs = _labelFontSize(z);

  // Opcional: ocultar a zoom muy bajo para evitar ruido
  const show = z >= 6;

  _provLabelMarkers.forEach((mk) => {
...
async function loadCantonesNbi() {
  ...
  if (urlGeo) {
    try {
      ...
      const lyr = L.geoJSON(gjFixed, {
        style: styleCantones(),
        pane: 'panePoligonos',
        onEachFeature: (feat, layer) => {
          const p = (feat && feat.properties) ? feat.properties : {};
          layer.bindPopup(popupCardHTML(p), { maxWidth: 360 });
        }
      });
    ...
    try {
      ...
      if (cb.checked) {
        if (!layers[layerKey]) {
          ...
          // Labels de provincias (si aplica)
          if (layerKey === 'provincias' && layers.provinciasLabels) {
            layers.provinciasLabels.addTo(map);
            updateProvinceLabelsByZoom(map);
          }
        ...
        if (layerKey === 'provincias' && layers.provinciasLabels) {
          map.removeLayer(layers.provinciasLabels);
        }
      }

      // Leyendas
      if (checkboxId === 'tgOtras') setLegendVisible('legendOtras', cb.checked);
      if (checkboxId === 'tgViolencia') setLegendVisible('legendViolencia', cb.checked);
    } catch (e) {
      console.error(e);
