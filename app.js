...
// Divide nombres largos en hasta 3 renglones (tipo Excel/Word)
function formatProvName(name) {
  ...
  const cut2 = Math.ceil((2 * total) / 3);

  const l1 = parts.slice(0, cut1).join(' ');
  const l2 = parts.slice(cut1, cut2).join(' ');
  const l3 = parts.slice(cut2).join(' ');

  return `${l1}<br>${l2}<br>${l3}`;
}

function makeProvLabelIcon(text, fontSizePx) {
  const safe = formatProvName(text);
  return L.divIcon({
...
function buildProvinciaLabels(polyLayer) {
  ...
  polyLayer.eachLayer((lyr) => {
    ...
    const m = L.marker(center, {
      ...
      interactive: false,
      keyboard: false,
      icon: makeProvLabelIcon(name, 12)
    });
    m._provName = name;
...
function updateProvinciaLabelsByZoom() {
  ...
  const z = map.getZoom();
  const show = z >= 6; // oculta cuando está muy lejos (evita ensuciar)
  const fontSize = Math.max(11, Math.min(16, 11 + (z - 6))); // escala suave
  ...
  provLabelMarkers.forEach((m) => {
    ...
    if (el) {
      ...
      // fallback si todavía no hay DOM
...
async function loadProvincias() {
  ...
  const poly = L.geoJSON(gj, { style: styleProvincias(), pane: 'panePoligonos' });
  const labels = buildProvinciaLabels(poly);

  const group = L.layerGroup([poly, labels]);
  layers.provincias = group;
  layers.provinciasPoly = poly;
  layers.provinciasLabels = labels;

  if (!map._provLabelsHook) {
    map.on('zoomend', updateProvinciaLabelsByZoom);
    map._provLabelsHook = true;
  }

  // ajusta según zoom actual
  setTimeout(updateProvinciaLabelsByZoom, 0);
...
async function loadCantonesNbi() {
  ...
  if (urlGeo) {
    try {
      ...
      const lyr = L.geoJSON(gjFixed, { style: styleCantones(), pane: 'panePoligonos' });
