...
// Colores por estado de priorización
const PRIOR_COLORS = { BUENO: '#32CD32', DETERIORO: '#ffa420', MALO: '#FF0000' };
let priorData = null;
let priorGroup = null;
let priorLayersByEstado = null;
...
function styleCantones() {
  return { color: '#de9c07', fillColor: '#ff0000', weight: 1.2, fill: true, fillOpacity: 0.15 };
}
...
async function loadCantonesNbi() {
  // Carga SOLO GeoJSON (estable en GitHub Pages). Evita TopoJSON para no provocar 404.
  const urlGeo = getSourceUrl('cantonesNbiGeo');
  if (!urlGeo) throw new Error('No hay URL para cantonesNbiGeo');
  ...
  const poly = L.geoJSON(gjFixed, {
    style: styleCantones,
    onEachFeature: onEachCantonNbi
  });

  // Etiquetas por zoom (si aplica)
...
(function init() {
  ...
  // Priorización Zona (puntos) - con sub-toggles por estado
  initPriorToggles();

  // ---- Priorización (sub-toggles por estado) ----
  async function ensurePriorData() {
    if (priorData) return priorData;
    const url = getSourceUrl('priorizacionZona');
    if (!url) throw new Error('No hay URL para Priorizacion_Zona');
    priorData = await fetchJson(url);
    return priorData;
  }

  function makePriorLayer(estadoUpper) {
    const data = priorData;
    const color = PRIOR_COLORS[estadoUpper] || '#7f8c8d';

    return L.geoJSON(data, {
      filter: (f) => {
        const p = f.properties || {};
        const est = (p.ESTADO_IE_ || p.ESTADO_IE || '').toString().trim().toUpperCase();
        return est === estadoUpper;
      },
      ...
      onEachFeature: (f, layer) => {
        const p = f.properties || {};
        const estadoPri = (p.ESTADO_IE_ || p.ESTADO_IE || 'N/D').toString().trim();
        const amie = p.AMIE || 'N/D';
        const nombre = p.NOM_INSTIT || p.NOMBRE_IE_ || 'Unidad Educativa';
        const tipoMaterial = p.TE_fin || p.TIPO_MATERI || '';
        const sosten = p.NOM_SOSTEN || p.SOSTENIMIE || '';
        const nivel = p.OFERTA_1 || p.NIVEL_EDU || '';
        const regimen = p.REGIMEN || '';
        const prov = p.DPA_DESPRO || '';
        const canton = p.DPA_DESCAN || '';
        const parroq = p.DPA_DESPAR || '';
        const zona = p.DA_ZONA || p.ZONA || '';
        const dist = p.DA_DIST || p.DISTRITO || '';

        layer.bindPopup(buildInfoPopup({
          titulo: nombre,
          subtitulo: 'AMIE: ' + amie,
          rows: [
            ['ESTADO', estadoPri],
  ...
  async function initPriorToggles() {
    const cbMaster = document.getElementById('tgPrior');
    const box = document.getElementById('priorSubtoggles');
    const cbB = document.getElementById('tgPriorBueno');
    const cbD = document.getElementById('tgPriorDeterioro');
    const cbM = document.getElementById('tgPriorMalo');

    const refresh = () => {
      if (!priorGroup) return;
      priorGroup.clearLayers();
      if (cbB && cbB.checked) priorGroup.addLayer(priorLayersByEstado.BUENO);
      if (cbD && cbD.checked) priorGroup.addLayer(priorLayersByEstado.DETERIORO);
      if (cbM && cbM.checked) priorGroup.addLayer(priorLayersByEstado.MALO);
    };
    ...
    const enable = async () => {
      try {
        ...
        await ensurePriorData();
        if (!priorGroup) {
          priorLayersByEstado = {
            BUENO: makePriorLayer('BUENO'),
            DETERIORO: makePriorLayer('DETERIORO'),
            MALO: makePriorLayer('MALO')
          };
          priorGroup = L.layerGroup();
        }
        refresh();
        priorGroup.addTo(map);
        setStatus('');
      } catch (e) {
        console.error(e);
        setStatus('No se pudo cargar Zona de Atención Prioritaria. Verifica data/Priorizacion_Zona.geojson');
    ...
    const disable = () => {
      if (box) box.style.display = 'none';
      if (priorGroup) map.removeLayer(priorGroup);
    };

    const onMasterChange = () => {
      if (cbMaster && cbMaster.checked) enable();
      else disable();
    };

    if (cbMaster) cbMaster.addEventListener('change', onMasterChange);
    if (cbB) cbB.addEventListener('change', refresh);
    if (cbD) cbD.addEventListener('change', refresh);
    if (cbM) cbM.addEventListener('change', refresh);
