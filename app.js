...
(function init() {
  ...
  // --- Servicios básicos: 4 toggles (Sí/No por Electricidad y Agua) ---
  // Carga única del GeoJSON y 4 sub-capas filtradas (más rápido y no rompe otras capas).
  let serviciosData = null;
  const serviciosSubLayers = {
    e_si: null,
    e_no: null,
    a_si: null,
    a_no: null
  ...
  const SERV_E_KEYS = ['Servicio_E','Servicio_e','SERVICIO_E','SERVICIO_EE','SERV_E'];
  const SERV_A_KEYS = ['Servicio_A','Servicio_a','SERVICIO_A','SERVICIO_AA','SERV_A'];
  ...
  async function ensureServiciosLoaded() {
  ...
  function buildServiciosSubLayer(filterFn, { fillColor, strokeColor }) {
  ...
  async function syncServiciosToggles() {
    const cbESi = document.getElementById('tgServE_SI');
    const cbENo = document.getElementById('tgServE_NO');
    const cbASi = document.getElementById('tgServA_SI');
    const cbANo = document.getElementById('tgServA_NO');

    // Si no existen en el DOM, no hacemos nada (evita romper el visor).
    if (!cbESi || !cbENo || !cbASi || !cbANo) return;

    // Si no hay ninguno prendido, solo limpia subcapas y no carga data.
    const anyOn = cbESi.checked || cbENo.checked || cbASi.checked || cbANo.checked;
    if (!anyOn) {
      Object.entries(serviciosSubLayers).forEach(([k, lyr]) => {
        if (lyr && map.hasLayer(lyr)) map.removeLayer(lyr);
      });
      return;
    }

    await ensureServiciosLoaded();

    // Electricidad - Sí (verde)
    if (cbESi.checked) {
      if (!serviciosSubLayers.e_si) {
        serviciosSubLayers.e_si = buildServiciosSubLayer(
          (p) => isYes(getProp(p, SERV_E_KEYS)),
          { fillColor: '#00c853', strokeColor: '#000' }
        );
      }
      if (!map.hasLayer(serviciosSubLayers.e_si)) serviciosSubLayers.e_si.addTo(map);
    } else if (serviciosSubLayers.e_si && map.hasLayer(serviciosSubLayers.e_si)) {
      map.removeLayer(serviciosSubLayers.e_si);
    }

    // Electricidad - No (rojo)
    if (cbENo.checked) {
      if (!serviciosSubLayers.e_no) {
        serviciosSubLayers.e_no = buildServiciosSubLayer(
          (p) => isNo(getProp(p, SERV_E_KEYS)),
          { fillColor: '#d50000', strokeColor: '#000' }
        );
      }
      if (!map.hasLayer(serviciosSubLayers.e_no)) serviciosSubLayers.e_no.addTo(map);
    } else if (serviciosSubLayers.e_no && map.hasLayer(serviciosSubLayers.e_no)) {
      map.removeLayer(serviciosSubLayers.e_no);
    }

    // Agua - Sí (azul)
    if (cbASi.checked) {
      if (!serviciosSubLayers.a_si) {
        serviciosSubLayers.a_si = buildServiciosSubLayer(
          (p) => isYes(getProp(p, SERV_A_KEYS)),
          { fillColor: '#2962ff', strokeColor: '#000' }
        );
      }
      if (!map.hasLayer(serviciosSubLayers.a_si)) serviciosSubLayers.a_si.addTo(map);
    } else if (serviciosSubLayers.a_si && map.hasLayer(serviciosSubLayers.a_si)) {
      map.removeLayer(serviciosSubLayers.a_si);
    }

    // Agua - No (naranja)
    if (cbANo.checked) {
      if (!serviciosSubLayers.a_no) {
        serviciosSubLayers.a_no = buildServiciosSubLayer(
          (p) => isNo(getProp(p, SERV_A_KEYS)),
          { fillColor: '#ff6d00', strokeColor: '#000' }
        );
      }
      if (!map.hasLayer(serviciosSubLayers.a_no)) serviciosSubLayers.a_no.addTo(map);
    } else if (serviciosSubLayers.a_no && map.hasLayer(serviciosSubLayers.a_no)) {
      map.removeLayer(serviciosSubLayers.a_no);
    }
  }

  // listeners
  ['tgServE_SI','tgServE_NO','tgServA_SI','tgServA_NO'].forEach((id) => {
    const el = document.getElementById(id);
    if (el) el.addEventListener('change', () => { syncServiciosToggles(); });
  });
  // sincroniza estado inicial
  syncServiciosToggles();
})();
