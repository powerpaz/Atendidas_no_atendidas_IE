...
async function loadProvincias() {
  ...
  const poly = L.geoJSON(gj, {
    ...
    onEachFeature: (feat, l) => {
      ...
      // Para Zona de Atención Prioritaria: mostrar el estado de priorización (BUENO/DETERIORO/MALO)
      // y evitar que "NOM_ESTADO" (p.ej. ACTIVA) se imponga en el popup.
      const estado = normEstado(p); // BUENO/DETERIORO/MALO
      const pp = { ...p };
      if (estado) {
        pp.ESTADO_IE_ = estado;
        pp.NOM_ESTADO = estado;
      }
