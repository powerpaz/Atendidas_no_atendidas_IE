...
function updateInlineLegends() {
  const cbOtras = document.getElementById('tgOtras');
  const boxOtras = document.getElementById('legendOtras');
  if (boxOtras && cbOtras) {
    boxOtras.style.display = cbOtras.checked ? 'block' : 'none';
    boxOtras.setAttribute('aria-hidden', cbOtras.checked ? 'false' : 'true');
  }
}
...
async function toggleLayer(checkboxId, loaderFn, layerKey) {
  ...
  const onChange = async () => {
    try {
      ...
      // Actualiza leyendas inline del panel
      if (typeof updateInlineLegends === 'function') updateInlineLegends();
    } catch (e) {
      console.error(e);
  ...
  if (cb.checked) {
    ...
    // Asegura estado inicial de leyendas aunque la capa venga apagada
    if (typeof updateInlineLegends === 'function') updateInlineLegends();
  }
}
...
(function init() {
  ...
  setStatus('');
  // Estado inicial de leyendas inline
  if (typeof updateInlineLegends === 'function') updateInlineLegends();
