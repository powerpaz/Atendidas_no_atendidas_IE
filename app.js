...
// Intenta varias URLs y devuelve el primer JSON que cargue
async function fetchFirstJson(urls) {
  ...
  for (const u of (urls || [])) {
    ...
    try {
      ...
      tried.push(String(u) + " -> " + (e && e.message ? e.message : e));
    }
  }
  const msg = tried.length ? tried.join(" | ") : "sin URLs";
  throw new Error("No se pudo cargar TopoJSON de cantones. " + msg);
}
...
async function loadCantonesNbiTopo() {
  const primary = getSourceUrl("cantonesNbiTopo");
  const fallbacks = (window.CANTONES_NBI_FALLBACK && Array.isArray(window.CANTONES_NBI_FALLBACK)) ? window.CANTONES_NBI_FALLBACK : [];
  const topo = await fetchFirstJson([primary, ...fallbacks]);
  ...
  const objName = (objects["Cantones con NBI mayor al 50%"] ? "Cantones con NBI mayor al 50%" : Object.keys(objects)[0]);
  if (!objName) throw new Error("TopoJSON sin objects");

  const geo = topojson.feature(topo, objects[objName]);
