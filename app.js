  ...
  toggleLayer(
    ...
    () => loadPointLayer('otrasNacionalidades', 'otrasNacionalidades', {
      ...
      symbolFn: (p) => {
        ...
        if (v <= 50)   return { radius: 4,  fill: 'rgba(248,113,113,.85)', stroke: '#000', fillOpacity: 0.75 };
        if (v <= 100)  return { radius: 7,  fill: 'rgba(239,68,68,.85)',  stroke: '#000', fillOpacity: 0.75 };
        // > 100
        return          { radius: 11, fill: 'rgba(185,28,28,.85)',  stroke: '#000', fillOpacity: 0.75 };
      },
      weight: 0.9
    }),
    'otrasNacionalidades'
