...
map.createPane('paneServicios');
map.getPane('paneServicios').style.zIndex = 430;
map.createPane('panePuntosOtras');
map.getPane('panePuntosOtras').style.zIndex = 460;
map.createPane('panePuntosViol');
map.getPane('panePuntosViol').style.zIndex = 470;
...
async function loadProvincias() {
  ...
  const paneName = (layerKey === 'violencia') ? 'panePuntosViol' : (layerKey === 'otrasNacionalidades') ? 'panePuntosOtras' : 'panePuntos';
...
(function init() {
  ...
  // --- Servicios básicos (agua y luz) con 4 toggles (Sí/No para cada servicio) ---
  // Se carga UNA sola vez y luego se muestran/ocultan subcapas filtradas.
  const SERV_FIELD_E = 'Servicio_E';
  const SERV_FIELD_A = 'Servicio_A';

  let serviciosData = null;
  let serviciosBuilt = false;

  const serviciosGroups = {
    eYes: L.layerGroup([], { pane: 'paneServicios' }),
    eNo:  L.layerGroup([], { pane: 'paneServicios' }),
    aYes: L.layerGroup([], { pane: 'paneServicios' }),
    aNo:  L.layerGroup([], { pane: 'paneServicios' })
  };

  function svgDivIcon(svg, size=16) {
    return L.divIcon({
      className: '',
      iconSize: [size, size],
      html: svg
  ...
  function iconNoElectric() {
    const svg = `
      <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 64 64">
        <circle cx="32" cy="32" r="30" fill="none" stroke="#d50000" stroke-width="6"/>
        <path d="M32 10c-8 0-14 6-14 14 0 5 2 8 5 11 3 3 4 5 4 8h10c0-3 1-5 4-8 3-3 5-6 5-11 0-8-6-14-14-14z"
              fill="none" stroke="#000" stroke-width="4" stroke-linejoin="round"/>
        <path d="M24 50h16" stroke="#000" stroke-width="4" stroke-linecap="round"/>
        <path d="M26 44h12" stroke="#000" stroke-width="4" stroke-linecap="round"/>
        <path d="M16 48L48 16" stroke="#d50000" stroke-width="6" stroke-linecap="round"/>
      </svg>`;
    return svgDivIcon(svg, 18);
  }
  ...
  function iconNoWater() {
    const svg = `
      <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 64 64">
        <circle cx="32" cy="32" r="30" fill="none" stroke="#d50000" stroke-width="6"/>
        <path d="M32 10C25 20 20 27 20 35c0 7 5 13 12 13s12-6 12-13c0-8-5-15-12-25z"
              fill="none" stroke="#000" stroke-width="4" stroke-linejoin="round"/>
        <path d="M16 48L48 16" stroke="#d50000" stroke-width="6" stroke-linecap="round"/>
      </svg>`;
    return svgDivIcon(svg, 18);
  ...
  function isYesSvc(v) {
    const s = String(v ?? '').trim().toLowerCase();
    return s === 'si' || s === 'sí' || s === 's' || s === '1' || s === 'true' || s === 'yes';
  }
  function isNoSvc(v) {
    const s = String(v ?? '').trim().toLowerCase();
  ...
  async function ensureServiciosBuilt() {
    if (serviciosBuilt) return;
    setStatus('Cargando servicios (agua y luz)...');

    const url = getUrl('servicios');
    try {
    ...
    // Construir subcapas por filtro
    const geojson = L.geoJSON(serviciosData, {
      pane: 'paneServicios',
      pointToLayer: (feature, latlng) => {
        const p = feature?.properties || {};
        const e = p[SERV_FIELD_E] ?? p['Servicio_e'] ?? p['SERVICIO_E'];
        const a = p[SERV_FIELD_A] ?? p['Servicio_a'] ?? p['SERVICIO_A'];

        const mkPopup = () => {
          // popup simple consistente
          const rows = Object.entries(p).slice(0, 15).map(([k,val]) =>
            `<tr><td style="padding:2px 6px;opacity:.75">${k}</td><td style="padding:2px 6px">${val ?? ''}</td></tr>`
          ).join('');
          return `<div style="max-width:320px"><table>${rows}</table></div>`;
        };

        // Nota: creamos un marcador "neutro" aquí y lo clonamos a grupos según filtro
        ...
        // Guardar refs para poder recrear como ícono en NO (Leaflet no clona bien markers entre layers)
        marker.__svcProps = { e, a, p, latlng, popupHtml: marker.getPopup().getContent() };
        return marker;
      }
    });

    // geojson trae todo, iteramos y distribuimos
    geojson.eachLayer((layer) => {
      const meta = layer.__svcProps;
      if (!meta) return;

      const eYes = isYesSvc(meta.e), eNo = isNoSvc(meta.e);
      const aYes = isYesSvc(meta.a), aNo = isNoSvc(meta.a);

      // Electricidad: Sí
      if (eYes) {
        serviciosGroups.eYes.addLayer(layer);
      }
      // Electricidad: No => ícono
      if (eNo) {
        const m = L.marker(meta.latlng, { icon: iconNoElectric(), pane: 'paneServicios' })
          .bindPopup(meta.popupHtml);
        serviciosGroups.eNo.addLayer(m);
      }
      // Agua: Sí
      if (aYes) {
        // para agua sí usamos círculo verde (ligeramente distinto)
        const m = L.circleMarker(meta.latlng, {
          radius: 4.5,
          fillColor: '#00c853',
          color: '#000',
          weight: 0.7,
        ...
        serviciosGroups.aYes.addLayer(m);
      }
      // Agua: No => ícono
      if (aNo) {
        const m = L.marker(meta.latlng, { icon: iconNoWater(), pane: 'paneServicios' })
          .bindPopup(meta.popupHtml);
        serviciosGroups.aNo.addLayer(m);
      }
    });
  ...
  function setServiciosGroupVisible(groupKey, visible) {
    const grp = serviciosGroups[groupKey];
    if (!grp) return;
    if (visible) {
      grp.addTo(map);
    } else {
  ...
  function wireServiciosToggle(cbId, groupKey) {
    const cb = document.getElementById(cbId);
    if (!cb) return;

    const handler = async () => {
      if (cb.checked) {
        await ensureServiciosBuilt();
        setServiciosGroupVisible(groupKey, true);
      } else {
        setServiciosGroupVisible(groupKey, false);
      }
    };

    cb.addEventListener('change', handler);
    if (cb.checked) handler();
  }

  wireServiciosToggle('tgServEYes', 'eYes');
  wireServiciosToggle('tgServENo', 'eNo');
  wireServiciosToggle('tgServAYes', 'aYes');
  wireServiciosToggle('tgServANo', 'aNo');
