...
// Para ayudar a diagnosticar archivos con proyección incorrecta (ej. UTM)
let _nbiAutoFitted = false;

function maybeFitAndWarnIfOutOfEcuador(lyr, label = 'Capa') {
  try {
    if (!lyr || typeof lyr.getBounds !== 'function') return;
    const b = lyr.getBounds();
    if (!b || !b.isValid()) return;
    const c = b.getCenter();
    // Ventana aproximada de Ecuador (lon/lat). Si cae MUY lejos, probablemente está proyectado.
    const looksLikeEcuador = (c.lat > -6.5 && c.lat < 3.5 && c.lng > -92 && c.lng < -70);
    const looksLikeLonLat = (Math.abs(c.lat) <= 90 && Math.abs(c.lng) <= 180);

    if (!looksLikeLonLat) {
      setStatus(`${label}: geometría parece NO estar en EPSG:4326 (centro: ${c.lat.toFixed(2)}, ${c.lng.toFixed(2)}). Revisa la proyección.`);
      if (!_nbiAutoFitted) {
    ...
    if (!looksLikeEcuador) {
      // No forzamos zoom siempre, pero dejamos pista
      setStatus(`${label}: se cargó, pero cae fuera del rango esperado (centro: ${c.lat.toFixed(2)}, ${c.lng.toFixed(2)}).`);
      if (!_nbiAutoFitted) {
        map.fitBounds(b, { padding: [20, 20] });
        _nbiAutoFitted = true;
      }
...
function styleCantones() {
  // Un poco más visible sobre basemap claro
  return { color: '#2ecc71', weight: 2.0, fill: true, fillOpacity: 0.10, opacity: 0.95 };
}
...
async function loadCantonesNbi() {
  // Preferimos GeoJSON (EPSG:4326). Si no existe, caemos a TopoJSON.
  const candidates = [];

  const urlGeo = getSourceUrl('cantonesNbiGeo');
  const urlTopo = getSourceUrl('cantonesNbiTopo');

  if (urlGeo) candidates.push(urlGeo);
  // Fallbacks por nombres comunes (por si el archivo quedó con doble extensión)
  candidates.push('data/cantones_nbi_mayor_50.geojson');
  candidates.push('data/cantones_nbi_mayor_50.topo.json.geojson');

  if (urlTopo) candidates.push(urlTopo);
  candidates.push('data/cantones_nbi_mayor_50.topo.json');

  let data = null;
  let used = null;

  for (const u of candidates) {
    data = await fetchJsonTry(u);
    if (data) { used = u; break; }
  }
  if (!data) throw new Error('No se pudo cargar Cantones NBI (verifica /data y el nombre del archivo)');

  // Auto-detectar formato
  if (data.type === 'FeatureCollection' || data.type === 'Feature') {
    const lyr = L.geoJSON(data, { style: styleCantones(), pane: 'panePoligonos' });
    layers.cantonesNbi = lyr;
    maybeFitAndWarnIfOutOfEcuador(lyr, 'Cantones NBI');
    return lyr;
  }

  if (data.type === 'Topology') {
    if (!window.topojson) {
      throw new Error('TopoJSON detectado pero topojson no está cargado (revisa index.html)');
    }
    const objName = Object.keys(data.objects || {})[0];
    if (!objName) throw new Error('TopoJSON sin objects');
    const geo = topojson.feature(data, data.objects[objName]);
  ...
  throw new Error(`Formato desconocido en Cantones NBI (${used})`);
}
