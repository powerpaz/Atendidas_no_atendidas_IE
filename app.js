...
const layers = {
  ...
  // Servicios básicos (se cargan una vez y se filtran en 4 subcapas)
  servicios_E_si: null,
  servicios_E_no: null,
  servicios_A_si: null,
  servicios_A_no: null
...
// ---------- Servicios básicos (filtros por SI/NO) ----------
let serviciosDataCache = null;

async function getServiciosData() {
  if (serviciosDataCache) return serviciosDataCache;
  const url = getSourceUrl('servicios');
  if (!url) throw new Error('No hay URL para servicios');
  serviciosDataCache = await fetchJson(url);
  return serviciosDataCache;
}

async function loadServiciosFiltered(layerKey, fieldName, wantYes, styleOpts = {}) {
  const gj = await getServiciosData();

  const fill = styleOpts.fillColor || (wantYes ? '#00c853' : '#d50000');
  const stroke = styleOpts.strokeColor || '#000';
  const radius = styleOpts.radius ?? 6;
  const weight = styleOpts.weight ?? 0.7;
  const fillOpacity = styleOpts.fillOpacity ?? 0.85;

  const lyr = L.geoJSON(gj, {
    pane: 'panePuntos',
    filter: (feat) => {
      const p = feat && feat.properties ? feat.properties : {};
      const raw = getProp(p, [fieldName, fieldName.toLowerCase(), fieldName.toUpperCase()]);
      const yes = isYes(raw);
      const no = isNo(raw);
...
(function init() {
  ...
  // Servicios básicos: 4 toggles (Electricidad SI/NO y Agua SI/NO)
  toggleLayer('tgServEYes', () => loadServiciosFiltered('servicios_E_si', 'Servicio_E', true), 'servicios_E_si');
  toggleLayer('tgServENo',  () => loadServiciosFiltered('servicios_E_no', 'Servicio_E', false), 'servicios_E_no');
  toggleLayer('tgServAYes', () => loadServiciosFiltered('servicios_A_si', 'Servicio_A', true), 'servicios_A_si');
  toggleLayer('tgServANo',  () => loadServiciosFiltered('servicios_A_no', 'Servicio_A', false), 'servicios_A_no');
})();
