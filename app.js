...
L.control.zoom({ position: 'topleft' })// .addTo(map); // labels provincias desactivados
...
async function loadProvincias() {
  ...
  const poly = L.geoJSON(gj, { style: styleProvincias(), pane: 'panePoligonos', onEachFeature: (feat, l) => { const p = feat.properties || {}; l.bindPopup(popupCardHTML(p), { maxWidth: 360 }); } });
    ...
    try {
      ...
      if (cb.checked) {
        if (!layers[layerKey]) {
          ...
          lyr// .addTo(map); // labels provincias desactivados
          ...
          layers[layerKey]// .addTo(map); // labels provincias desactivados
        }
      } else {
