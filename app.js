...
function getSourceUrl(key) {
  ...
  if (typeof DEFAULT_RELEASE_BASE !== 'undefined' && DEFAULT_RELEASE_BASE) {
    const map = {
      ...
      cantonesNbiTopo: null, // deshabilitado
...
function styleCantones() {
  return { color: '#de9c07', fillColor: (PRIOR_COLORS[estadoUpper] || '#7c3aed'), weight: 1.2, fill: true, fillOpacity: 0.15 };
}
...
function popupCardHTML(p) {
  ...
  const candidates = [
    ['ESTADO', ['ESTADO_IE_','ESTADO_IE','NOM_ESTADO','ESTADO']],
...
async function loadCantonesNbi() {
  ...
  // Si llega aquí, el GeoJSON falló. Para evitar 404 y no “romper” el visor,
  // NO intentamos TopoJSON (no se usa en este proyecto).
  throw new Error('No se pudo cargar Cantones con NBI > 50%. Verifica data/cantones_nbi_mayor_50.geojson');
